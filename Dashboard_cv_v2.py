...
# # AWS S3 configuration
# S3_BUCKET_NAME = "trialbucket-cv"
# S3_FOLDERS = ["detections/cam1/", "detections/cam2/"]
...
# # Función para cargar y procesar los datos
# def load_data(s3_folders):
#     pattern = r"Zone_(\d+)_person_(\d+)_(\d{8})_(\d{6})"
#     detections = []
    
#     for folder in s3_folders:
#         for key in list_s3_objects(S3_BUCKET_NAME, folder):
#             filename = os.path.basename(key)
#             match = re.match(pattern, filename)
#             if match:
#                 zone, person_id, date, time = match.groups()
...
#         if st.button("Recargar datos"):
#             st.session_state.detections = load_data(S3_FOLDERS)
#             st.success("Datos recargados exitosamente!")

# # Resto del código (funciones y lógica principal) se mantiene igual
# # Modificar referencias a `S3_FOLDER` en todo el código por `S3_FOLDERS` donde corresponda.
