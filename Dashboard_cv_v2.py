...
# Función para cargar y procesar los datos (sin caché)
def load_data(s3_folder):
    pattern = r"Zone_(\d+)_person_(\d+)_(\d{8})_(\d{6})"
    detections = []
...
# Función mejorada para mostrar la imagen principal y las miniaturas clickeables
def show_image_and_info(index, filenames):
    if 0 <= index < len(filenames):
        col1, col2 = st.columns([3, 1])
        ...
        with col2:
            ...
            for i in range(index + 1, min(index + 4, len(filenames))):
                ...
                try:
                    ...
                    # Usar st.image con on_click para hacer la imagen clickeable
                    if st.image(thumbnail, width=100, output_format="PNG"):
                        st.session_state.current_index = i
                        st.experimental_rerun()
