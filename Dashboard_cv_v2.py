...
def show_alerts_section():
    ...
    col1, col2 = st.columns(2)
    with col1:
        start_date = st.date_input("Fecha de inicio", start_time.date(), key="start_date")
    with col2:
        end_date = st.date_input("Fecha de fin", end_time.date(), key="end_date")
    ...
    if filtered_detections:
        ...
        with col2:
            for i in range(3):
                index = (st.session_state.current_index + i) % len(filenames)
                thumbnail = show_image_and_info(index, filenames)
                if thumbnail:
                    thumbnail.thumbnail((100, 100))
                    if st.image(thumbnail, caption=f"Image {index+1}", use_column_width=True):
