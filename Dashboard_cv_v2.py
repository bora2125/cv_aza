...
#     st.write(f"Total de imágenes cargadas: {len(detections)}")

#     # Mostrar imágenes de alertas
#     if detections:
#         filenames = [d[3] for d in detections]
...
#     events_by_hour, events_by_zone = process_data_for_visualization(detections)
...
import cv2
import numpy as np
from polygon_drawer import create_polygon_drawer
...
# Función para mostrar la sección de Control Room (ROI)
def show_control_room_section():
    st.header("Control room (ROI)")
    facility, section, camera, event_type, start_date, end_date = filters()

    # Cargar la imagen más reciente de S3
    if 'detections' not in st.session_state:
        st.session_state.detections = load_data(S3_FOLDER)

    if st.session_state.detections:
        most_recent_image_key = st.session_state.detections[0][3]
        try:
            response = s3_client.get_object(Bucket=S3_BUCKET_NAME, Key=most_recent_image_key)
            image_data = response['Body'].read()
        ...
        # Llamar a la función de creación de polígonos
        polygon_points = create_polygon_drawer(img)

        # Mostrar coordenadas del polígono
        if polygon_points:
            st.write("Coordenadas del polígono:")
            st.write(polygon_points)
